...
```{ojs}
//| include: false
// load data as CSVs directly (NOUAKCHOTT DATA)
...
// hard code fe events since Nouakchott output did not include the standard flood-events.csv file, and instead, only included the scan-calculations.html sheet pdf output for this plot
fe = [
  {
    begin_year: 1995,
    begin_month: 9,
    ...
    line1: "SEPTEMBER 1995: Large event, Heavy rain",
    line2: "7 days, 0 fatalities, 95,000 displaced",
    line3: ""
  }
];
...
// function with d3.js that plots forest remaining in HECTARES over time to create deforestation_area2 chart

// note: pixel size calculation:
// Hansen Global Forest Change dataset in GEE has a native pixel resolution of 30.92 meters, even though it is commonly cited as 30 meters https://developers.google.com/earth-engine/datasets/catalog/UMD_hansen_global_forest_change_2024_v1_12

// pixel area calculation:
// 30.92m * 30.92m = 956.05 m²; 956.05 m² / 10,000 = 0.095605 hectares per pixel

// backend exports use scale: 30.92 (see gee_fun.py line 323, 335), so this conversion factor is correct: https://github.com/rosemaryturtle/city-scan-automation/blob/main/backend/gee_fun.py 
...
function plot_deforestation_area1(deforestation_area, {
  ...
  width = plotWidth,
  xLabel = "Year",
  yLabel = "Forest Area Remaining (hectares)",
  color = "black",
  pixelSizeHectares = 0.095605  // backend export: 30.92m x 30.92m pixel = 0.095605 ha https://github.com/rosemaryturtle/city-scan-automation/blob/main/backend/gee_fun.py 
} = {}) {
  ...
  // convert pixels to hectares and add to data
  const dataWithHectares = deforestation_area.map(d => ({
    ...d,
    forest_remaining_ha: d.forest_remaining * pixelSizeHectares
  }));
  
  // year range
  const minYear = d3.min(dataWithHectares, d => d.year);
  const maxYear = d3.max(dataWithHectares, d => d.year);
  
  // scales
  const xScale = d3.scaleLinear()
    .domain(d3.extent(dataWithHectares, d => d.year))
    .range([0, innerWidth]);
    
  const yScale = d3.scaleLinear()
    .domain([
      d3.min(dataWithHectares, d => d.forest_remaining_ha) * 0.995,  // slight padding
      d3.max(dataWithHectares, d => d.forest_remaining_ha) * 1.002
    ])
    .range([innerHeight, 0]);
  ...
  // line (with defined function to skip null values)
  const line = d3.line()
    .defined(d => d.forest_remaining_ha !== null && !isNaN(d.forest_remaining_ha))
    .x(d => xScale(d.year))
    .y(d => yScale(d.forest_remaining_ha));
    
  g.append("path")
    .datum(dataWithHectares)
    .attr("fill", "none")
    .attr("stroke", color)
    .attr("stroke-width", Math.max(1.5, Math.min(3, width / 400)))
    .attr("d", line);
  ...
  // dots (filter out null values)
  g.selectAll(".dot")
    .data(dataWithHectares.filter(d => d.forest_remaining_ha !== null && !isNaN(d.forest_remaining_ha)))
    .enter().append("circle")
    .attr("class", "dot")
    .attr("cx", d => xScale(d.year))
    .attr("cy", d => yScale(d.forest_remaining_ha))
    .attr("r", Math.max(2, Math.min(3, width / 200)))
    .attr("fill", color)
    .style("cursor", "default")
    .on("mouseover", function(event, d) {
      ...
      tooltip.html(`<div>Year: ${d.year}</div><div>${yLabel}: ${d.forest_remaining_ha.toLocaleString('en-US', {maximumFractionDigits: 0})} ha</div>`)
        .style("left", (event.pageX - 60) + "px")
        .style("top", (event.pageY - 60) + "px")
        .transition()
        .duration(200)
  ...
  svg.append("text")
    ...
    .attr("font-family", "system-ui")
    .attr("font-weight", "normal")
    .text(`Forest Remaining Over Time, ${minYear}-${maxYear}`);
...
function plot_deforestation_area2(deforestation_area, {
  ...
  width = plotWidth,
  xLabel = "Year",
  yLabel = "Cumulative Forest Loss (hectares)",
  color = "black",
  pixelSizeHectares = 0.095605  // backend export: 30.92m x 30.92m pixel = 0.095605 ha, https://github.com/rosemaryturtle/city-scan-automation/blob/main/backend/gee_fun.py
} = {}) {
  ...
  const dataWithHectares = deforestation_area.map(d => ({
    ...d,
    cumulative_deforested_ha: d.cumulative_deforested * pixelSizeHectares
  }));
  ...
  const yScale = d3.scaleLinear()
    .domain([0, d3.max(dataWithHectares, d => d.cumulative_deforested_ha) * 1.1])
    .range([innerHeight, 0]);
  ...
  // line (with defined function to skip null values)
  const line = d3.line()
    .defined(d => d.cumulative_deforested_ha !== null && !isNaN(d.cumulative_deforested_ha))
    .x(d => xScale(d.year))
    .y(d => yScale(d.cumulative_deforested_ha));
  ...
  // dots (filter out null values)
  g.selectAll(".dot")
    .data(dataWithHectares.filter(d => d.cumulative_deforested_ha !== null && !isNaN(d.cumulative_deforested_ha)))
    .enter().append("circle")
    .attr("class", "dot")
    .attr("cx", d => xScale(d.year))
    .attr("cy", d => yScale(d.cumulative_deforested_ha))
    .attr("r", Math.max(2, Math.min(3, width / 200)))
    .attr("fill", color)
    .style("cursor", "default")
    .on("mouseover", function(event, d) {
      ...
      tooltip.html(`<div>Year: ${d.year}</div><div>${yLabel}: ${d.cumulative_deforested_ha.toLocaleString('en-US', {maximumFractionDigits: 0})} ha</div>`)
        .style("left", (event.pageX - 60) + "px")
        .style("top", (event.pageY - 60) + "px")
        .transition()
        .duration(200)
  ...
  svg.append("text")
    ...
    .attr("font-family", "system-ui")
    .attr("font-weight", "normal")
    .text(`Forest Loss Over Time, ${minYear}-${maxYear}`);
...
// function with d3.js that plots deforestation_area data for annual deforestation in HECTARES
...
function plot_deforestation_area3(deforestation_area, {
    ...
    width = plotWidth,
    xLabel = "Year",
    yLabel = "Annual Forest Loss (hectares)",
    pixelSizeHectares = 0.095605  // backend export: 30.92m x 30.92m pixel = 0.095605 ha https://github.com/rosemaryturtle/city-scan-automation/blob/main/backend/gee_fun.py 
  } = {}) {
    ...
    // convert pixels to hectares for annual deforestation
    const dataWithHectares = sortedData.map(d => ({
      ...d,
      deforested_this_year_ha: d.deforested_this_year * pixelSizeHectares
    }));
    ...
    const xScale = d3.scaleBand()
      .domain(dataWithHectares.map(d => d.year))
      .range([0, innerWidth])
      .padding(0.15);
      
    // dynamic y-scale based on data
    const maxValue = d3.max(dataWithHectares, d => d.deforested_this_year_ha);
    const yScale = d3.scaleLinear()
    ...
    g.selectAll(".data-bar")
      .data(dataWithHectares.filter(d => d.deforested_this_year_ha > 0))
      .enter().append("rect")
      .attr("class", "data-bar")
      .attr("x", d => xScale(d.year))
      .attr("width", xScale.bandwidth())
      .attr("y", d => yScale(d.deforested_this_year_ha))
      .attr("height", d => innerHeight - yScale(d.deforested_this_year_ha))
      .attr("fill", barColor)
      .attr("fill-opacity", 1)
      ...
      .on("mouseover", function(event, d) {
        ...
        tooltip.html(`<div>Year: ${d.year}</div><div>Forest Lost: ${d.deforested_this_year_ha.toLocaleString('en-US', {maximumFractionDigits: 1})} ha</div>`)
          .style("left", (event.pageX - 60) + "px")
          .style("top", (event.pageY - 70) + "px")
          .transition()
          .duration(200)
    ...
    const numYears = dataWithHectares.length;
    const tickInterval = Math.max(1, Math.ceil(numYears / Math.floor(width / 50)));
    const tickValues = dataWithHectares
    ...
    svg.append("text")
      ...
      .attr("font-size", "18px") 
      .attr("font-family", "system-ui")
      .text(`Annual Deforestation, ${minYear}-${maxYear}`);
...
// function with d3.js that plots flood events to create (scatter) flood event chart (i.e., Large flood events in xxxx)
function plot_fe(fe, {
  cityName = "City",
  width = plotWidth,
  yLabel = "Year",
...
//| echo: false
{
  // auto-generate insights from data (as of 17dic2025, this is NOT working - Nouakchott information is hardcoded for the sake of delivery, but the plan is to scale this for automatic insight generation)
  const popInsights = {
    latestPop: pg[pg.length - 1].population.toLocaleString('en-US', {maximumFractionDigits: 0}),
    latestYear: pg[pg.length - 1].yearName,
    earliestYear: pg[0].yearName,
    totalGrowth: ((pg[pg.length - 1].population - pg[0].population) / pg[0].population * 100).toFixed(1),
    avgGrowth: (pg.slice(1).reduce((sum, d) => sum + d.populationGrowthPercentage, 0) / (pg.length - 1)).toFixed(2)
  };

  const builtUpInsights = {
    earliestArea: uba[0].builtUpArea,
    latestArea: uba[uba.length - 1].builtUpArea,
    earliestYear: uba[0].year,
    latestYear: uba[uba.length - 1].year,
    growth: (((uba[uba.length - 1].builtUpArea - uba[0].builtUpArea) / uba[0].builtUpArea) * 100).toFixed(2)
  };

  const pvInsights = {
    avgPV: (pv.reduce((sum, d) => sum + d.mean, 0) / 12).toFixed(2),
    maxMonth: ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"][pv.reduce((max, d, i) => d.mean > pv[max].mean ? i : max, 0)],
    minMonth: ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"][pv.reduce((min, d, i) => d.mean < pv[min].mean ? i : min, 0)]
  };

  const riskInsights = {
    totalEvents: fe.length,
    totalDisplaced: fe.reduce((sum, d) => sum + d.DISPLACED, 0).toLocaleString(),
    veryLarge: fe.filter(d => d.severity === "Very large event").length
  };
  
  const landCoverInsights = (() => {
    const lcTypes = lc.map(d => ({
      type: d.type,
      area: d.area
  ...
  return html`<div style="max-width: 1400px; margin: 0 auto; font-family: system-ui;">
    ...
    <div style="display: grid; grid-template-columns: 45% 55%; gap: 30px; align-items: start; margin: 40px 0; border-top: 4px solid #6ccff6; padding-top: 20px;">
      
      <div style="font-size: 13px; line-height: 1.5; color: #333;">
        
        <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 12px;">
          <div style="width: 40px; height: 40px; background: #6ccff6; border-radius: 4px; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 20px;">2</div>
          <h3 style="margin: 0; font-size: 18px; font-weight: 600;">Setting the Context</h3>
        </div>
        
        <p style="margin: 12px 0; color: #555; line-height: 1.6;">
          Nouakchott is located in southwestern Mauritania, along the Atlantic coast at the western edge of the Sahara Desert and serves as the country's capital and largest city. This analysis is for the city and its surrounding areas.
        </p>
           
      </div>

      <div style="display: flex; justify-content: center; align-items: center;">
        <img src="${await FileAttachment("Nouakchott_in_Mauritania.svg").url()}" 
             style="max-width: 50%; height: auto;" 
             alt="Map of Nouakchott, Mauritania">
      </div>
      
    </div>
    
    <!-- SECTION 3: POPULATION AND DEMOGRAPHIC TRENDS -->
    <div style="display: grid; grid-template-columns: 45% 55%; gap: 30px; align-items: start; margin: 40px 0; border-top: 4px solid #659a76; padding-top: 20px;">
      
      <div style="font-size: 13px; line-height: 1.5; color: #333;">
        
        <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 12px;">
          <div style="width: 40px; height: 40px; background: #659a76; border-radius: 4px; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 20px;">3</div>
          <h3 style="margin: 0; font-size: 18px; font-weight: 600;">Population and Demographic Trends</h3>
        </div>
        
        <p style="margin: 12px 0; color: #555; line-height: 1.6;">
          Nouakchott's population has grown by <strong>${popInsights.totalGrowth}%</strong> from ${popInsights.earliestYear} to ${popInsights.latestYear} with an average growth rate of <strong>${popInsights.avgGrowth}%</strong> annually. In ${popInsights.latestYear}, the area had a population of <strong>${popInsights.latestPop}</strong> people. The population mainly concentrates around the main urban centralities, including Sebkha, Arafat, and Dat Naim.
        </p>
           
      </div>

      <div>
        ${plot_pgp(pg, {width: 600, cityName: "Nouakchott"})}
      </div>
      
    </div>

    <!-- SECTION 4: ECONOMIC ACTIVITY -->
    <div style="display: grid; grid-template-columns: 45% 55%; gap: 30px; align-items: start; margin: 40px 0; border-top: 4px solid #f5a5c6; padding-top: 20px;">
     
      <div style="font-size: 13px; line-height: 1.5; color: #333;">
        
        <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 12px;">
          <div style="width: 40px; height: 40px; background: #f5a5c6; border-radius: 4px; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 20px;">4</div>
          <h3 style="margin: 0; font-size: 18px; font-weight: 600;">Economic Activity</h3>
        </div>
      ...
      <div>
        ${plot_rwi_area(rwi_area, {width: 600, cityName: "Nouakchott"})}
      </div>
      
    </div>

    <!-- SECTION 5: BUILT FORM -->
    <div style="display: grid; grid-template-columns: 45% 55%; gap: 30px; align-items: start; margin: 40px 0; border-top: 4px solid #fcf8c8; padding-top: 20px;">
      
      <div style="font-size: 13px; line-height: 1.5; color: #333;">
        
        <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 12px;">
          <div style="width: 40px; height: 40px; background: #fcf8c8; border-radius: 4px; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 20px;">5</div>
          <h3 style="margin: 0; font-size: 18px; font-weight: 600;">Built Form</h3>
        </div>
        
        <p style="margin: 12px 0; color: #555; line-height: 1.6;">
          Since 1985, Nouakchott's built-up area has increased by <strong>323.27%</strong>, growing from <strong>28.6 km²</strong> in 1985 to <strong>121.05 km²</strong> in 2015. Most recent expansion has occurred mainly toward the east and southeast, as well as north of Ksar and Tevragh-Zeina and west of Sebkha. Most of the land outside the built-up area is <strong>bare / spare vegetation</strong>, followed by <strong>permanent water bodies</strong>, and <strong>shrubland</strong>. Schools and health facilities are fairly well distributed throughout the area.
        </p>
           
      </div>

      <div>
        ${plot_ubaa(uba, {width: 600, cityName: "Nouakchott"})}
      </div>
      
    </div>
    
    <!-- SECTION 6: CLIMATE CONDITIONS -->
    <div style="margin: 40px 0; border-top: 4px solid #aaff00; padding-top: 20px;">
      
      <div style="font-size: 13px; line-height: 1.5; color: #333; margin-bottom: 20px;">
        
        <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 12px;">
          <div style="width: 40px; height: 40px; background: #aaff00; border-radius: 4px; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 20px;">6</div>
          <h3 style="margin: 0; font-size: 18px; font-weight: 600;">Climate Conditions</h3>
        </div>
        
        <p style="margin: 12px 0; color: #555; line-height: 1.6;">
          Solar energy potential is <strong>excellent</strong> in Nouakchott. Air quality is <strong>poor</strong> across the area, and the concentration of particulate matter <strong>exceeds</strong> WHO safety standards. Summer surface temperatures display a clear coastal-to-inland gradient, with cooler conditions along the coast and progressively higher temperatures inland and toward the east.
        </p>
      
      </div>

      <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 30px;">
        <div style="overflow: hidden;">${plot_pv_alt(pv, {width: 420, cityName: "Nouakchott"})}</div>
        <div style="overflow: hidden;">${plot_aq(aq_area, {width: 420, cityName: "Nouakchott"})}</div>
        <div style="overflow: hidden;">${plot_summer_area(summer_area, {width: 420, cityName: "Nouakchott"})}</div>
      </div>
      
    </div>
    
    <!-- SECTION 7: RISK IDENTIFICATION -->
    <div style="margin: 40px 0; border-top: 4px solid #eb8b25; padding-top: 20px;">
      
      <div style="font-size: 13px; line-height: 1.5; color: #333; margin-bottom: 20px;">
        
        <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 12px;">
          <div style="width: 40px; height: 40px; background: #eb8b25; border-radius: 4px; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 20px; color: white;">7</div>
          <h3 style="margin: 0; font-size: 18px; font-weight: 600;">Risk Identification</h3>
        </div>
        
        <p style="margin: 12px 0; color: #555; line-height: 1.6;">
          Floods pose some risk in Nouakchott. Since 1995, there has been <strong>${riskInsights.totalEvents} large flood event${riskInsights.totalEvents > 1 ? 's' : ''}</strong>, displacing a total of <strong>${riskInsights.totalDisplaced}</strong> people. As of 2015, <strong>32.68%</strong> of the built-up area was exposed to <strong>combined</strong> riverine, surface water, and coastal <strong>flooding</strong>. There is <strong>very low landslide susceptibility</strong> throughout, while <strong>liquefaction</strong> in the area is <strong>medium to high</strong>. <strong>Burnability</strong> is limited in extent but spatially concentrated along the urban periphery.
        </p>
            
      </div>

      <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 30px;">
        <div style="overflow: hidden;">${plot_fe(fe, {width: 420, cityName: "Nouakchott"})}</div>
        <div style="overflow: hidden;">${plot_ls_area(ls_area, {width: 420, cityName: "Nouakchott"})}</div>
        <div style="overflow: hidden;">${plot_l_area(l_area, {width: 420, cityName: "Nouakchott"})}</div>
      </div>
      
    </div>

    <!-- SECTION 8: LOCAL INSTITUTIONS AND PLANNING -->
    <div style="display: grid; grid-template-columns: 45% 55%; gap: 30px; align-items: start; margin: 40px 0; border-top: 4px solid #af5881; padding-top: 20px;">
      
      <div style="font-size: 13px; line-height: 1.5; color: #333;">
        
        <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 12px;">
          <div style="width: 40px; height: 40px; background: #af5881; border-radius: 4px; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 20px;">8</div>
          <h3 style="margin: 0; font-size: 18px; font-weight: 600;">Local Institutions and Planning</h3>
        </div>
...
chart_pga = plot_pga(pg, {cityName: "Nouakchott, Mauritania"})

// use plot_pgp function to create the pgp chart
chart_pgp = plot_pgp(pg, {cityName: "Nouakchott, Mauritania"})

// use plot_pas function to create the pas chart
chart_pas = plot_pas(pas, {cityName: "Nouakchott, Mauritania"})

// use plot_pas_pyramid function to create the pas pyramid chart
chart_pas_pyramid = plot_pas_pyramid(pas, {cityName: "Nouakchott, Mauritania"})

// use plot_age function to create the age (using pas data) treemap chart
chart_age = plot_age(pas, {cityName: "Nouakchott, Mauritania"})

// use plot_dependency function to create the dependency bubble chart (using pas data)
chart_dependency = plot_dependency(pas, {cityName: "Nouakchott, Mauritania"})
```

### ECONOMIC ACTIVITY
```{ojs}
// use plot_rwi_area function to create the rwi (i.e., relative wealth index, rwi) chart
chart_rwi_area = plot_rwi_area(rwi_area, {cityName: "Nouakchott, Mauritania"})
```
...
chart_uba_area = plot_uba_area(uba_area, {cityName: "Nouakchott, Mauritania"})

// use plot_ubaa function to create the ubaa chart
chart_ubaa = plot_ubaa(uba, {cityName: "Nouakchott, Mauritania"})

// use plot_ubap function to create the ubap chart
chart_ubap = plot_ubap(uba, {cityName: "Nouakchott, Mauritania"})

// use plot_lc to make the lc chart
chart_lc = plot_lc(lc, {cityName: "Nouakchott, Mauritania"})
...
chart_uddm = plot_uddm(pg, uba, pug, {cityName: "Nouakchott, Mauritania"})
```
...
chart_aq = plot_aq(aq_area, {cityName: "Nouakchott, Mauritania"})

// use plot_summer_area function to create the summer surface temperature area chart
chart_summer_area = plot_summer_area(summer_area, {cityName: "Nouakchott, Mauritania"})

// use plot_ndvi_area function to create the ndvi (i.e., green space, ndvi) chart
chart_ndvi_area = plot_ndvi_area(ndvi_area, {cityName: "Nouakchott, Mauritania"})

// use plot_deforestation_area1 function to create the deforestation_area1 (line) chart
// chart_deforestation_area1 = plot_deforestation_area1(deforestation_area, {cityName: "Nouakchott, Mauritania"})

// use plot_deforestation_area2 function to create the deforestation_area2 (line) chart
// chart_deforestation_area2 = plot_deforestation_area2(deforestation_area, {cityName: "Nouakchott, Mauritania"})

// use plot_deforestation_area3 function to create the deforestation_area3 (bar) chart (i.e., annual deforestation in hectares)
// chart_deforestation_area3 = plot_deforestation_area3(deforestation_area, {cityName: "Nouakchott, Mauritania"})

// use plot_deforestation_area4 function to create the deforestation_area4 (bar) chart (i.e., annual deforestation as a percentage of total deforestation)
// chart_deforestation_area4 = plot_deforestation_area4(deforestation_area, {cityName: "Nouakchott, Mauritania"})
...
// chart_deforestation_area5 = plot_deforestation_area5(deforestation_area, {cityName: "Nouakchott, Mauritania"})
...
// chart_deforestation_area6 = plot_deforestation_area6(deforestation_area, {cityName: "Nouakchott, Mauritania"})
```

### RISK IDENTIFCATION
```{ojs}
// use plot_fe function to create the fe chart
chart_fe = plot_fe(fe, {cityName: "Nouakchott, Mauritania"})

// use plot_fu function to create the fu chart
chart_fu = plot_fu(fu, {cityName: "Nouakchott, Mauritania"})

// use plot_pu function to create the pu chart
chart_pu = plot_pu(pu, {cityName: "Nouakchott, Mauritania"})

// use plot_cu function to create the cu chart
chart_cu = plot_cu(cu, {cityName: "Nouakchott, Mauritania"})

// use plot_comb function to create the comb chart
chart_comb = plot_comb(comb, pu, fu, cu, {cityName: "Nouakchott, Mauritania"})

// use plot_e function to create the e (i.e., elevation) chart
chart_e = plot_e(e, {cityName: "Nouakchott, Mauritania"})

// use plot_s function to create the s (i.e., slope) chart
chart_s = plot_s(s, {cityName: "Nouakchott, Mauritania"})
...
chart_ls_area= plot_ls_area(ls_area, {cityName: "Nouakchott, Mauritania"})
...
chart_l_area= plot_l_area(l_area, {cityName: "Nouakchott, Mauritania"})

// use plot_ee function to create the ee chart
// chart_ee = plot_ee(ee, {cityName: "Nouakchott, Mauritania"})

// use plot_fwi function to create the fwi chart
chart_fwi = plot_fwi(fwi, {cityName: "Nouakchott, Mauritania"})

// use plot_fwi_d function to create the fwi_d chart
plot_fwi_d(fwi, {cityName: "Nouakchott, Mauritania"})
```
